...
macro_rules! const_destructure {
    ...
    (@tuple (); ($($index_rest:tt),*) => ($($ty:tt),*); let ($($index:tt: $var:pat_param),*) = $value:expr) => {
    ...
    (@tuple ($var_head:pat_param $(,$var_tail:pat_param)*); () => ($($ty:tt),*); let ($($index:tt: $var:pat_param),*) = $value:expr) => {
        compile_error!("tuple arity is larger than the maximum supported arity 12")
    };
    (@tuple ($var_head:pat_param $(,$var_tail:pat_param)*); ($index_head:tt $(,$index_tail:tt)*) => ($($ty:tt),*); let ($($index:tt: $var:pat_param),*) = $value:expr) => {
